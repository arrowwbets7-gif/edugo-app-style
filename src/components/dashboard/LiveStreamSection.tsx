...
import {
  Radio, Send, Pin, Trash2, Plus, Loader2, X, Users
} from "lucide-react";
...
const LiveStreamSection = ({ isTeacher = false }: LiveStreamSectionProps) => {
  ...
  const createStream = async (e: React.FormEvent) => {
    ...
    if (!ytId || !title.trim()) {
      toast.error("Valid title and YouTube Live URL required");
      return;
    }
    setSaving(true);
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={() => setActiveStream(null)} className="text-xs">
          ...
          <ScrollArea className="h-60">
            <div className="p-3 space-y-2">
              {chatMessages.map((msg) => (
                <div key={msg.id} className={`flex items-start gap-2 text-sm group ${msg.is_pinned ? "bg-accent/5 -mx-1 px-1 rounded" : ""}`}>
  ...
  return (
    <div className="space-y-3">
      {isTeacher && !showCreate && (
        <Button onClick={() => setShowCreate(true)} className="w-full bg-red-500 hover:bg-red-600 text-white">
          <Plus className="w-4 h-4 mr-2" /> Start Live Class
        </Button>
      )}
        ...
        <Card className="border-red-500/20">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg flex items-center gap-2">
              <Radio className="w-5 h-5 text-red-500" /> Start Live Class
            </CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={createStream} className="space-y-3">
              <div className="space-y-2">
                <Label>Title *</Label>
                <Input placeholder="Live class title" value={title} onChange={(e) => setTitle(e.target.value)} required maxLength={200} />
              </div>
              <div className="space-y-2">
                <Label>Subject</Label>
              ...
              <div className="space-y-2">
                ...
                <Input placeholder="https://youtube.com/live/..." value={ytUrl} onChange={(e) => setYtUrl(e.target.value)} required />
              </div>
              <div className="flex gap-2">
                <Button type="submit" disabled={saving} className="flex-1 bg-red-500 hover:bg-red-600 text-white">
                  {saving ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Radio className="w-4 h-4 mr-2" />} Go Live
                </Button>
      ...
      {streams.length === 0 ? (
        ...
        streams.map((stream) => (
          <Card key={stream.id} className={`border-border/50 cursor-pointer hover:shadow-md transition-shadow ${stream.status === "live" ? "border-red-500/30" : ""}`}
            ...
            <CardContent className="pt-4 pb-3">
              <div className="flex items-center justify-between gap-2">
                ...
                {isTeacher && stream.status === "live" && (
                  <Button variant="destructive" size="sm" onClick={(e) => { e.stopPropagation(); endStream(stream.id); }}>
                    End
                  </Button>
                )}
              </div>
